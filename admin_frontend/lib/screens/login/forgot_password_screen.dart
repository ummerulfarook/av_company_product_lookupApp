...
class _ForgotPasswordScreenState extends State<ForgotPasswordScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      context.push(Uri(path: RouteConstants.resetPassword, queryParameters: {'email': email}).toString());
    } catch (e) {
      if (!mounted) return;
      ...
      body: Container(
        ...
        child: SafeArea(child: Padding(
          ...
          child: Form(key: _formKey, child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            const Icon(Icons.lock_reset_rounded, size: 80, color: AppTheme.primary).animate().scale(),
            ...
            TextButton(onPressed: () => context.push(RouteConstants.resetPassword), child: Text('Already have a token?', style: TextStyle(color: AppTheme.primary, fontWeight: FontWeight.w600))),
          ])),
        )),
      ),
    );
